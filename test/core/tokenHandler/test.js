...
QUnit.cases.init([
    ...
    const instance = tokenHandlerFactory();
    assert.expect(1);
    assert.equal(
...
QUnit.module('behaviour', {
    beforeEach: function() {
        this.cachedModuleConfig = module.config()['core/tokenHandler'];
    },
    afterEach: function() {
        module.config()['core/tokenHandler'] = this.cachedModuleConfig;
    }
});
...
QUnit.cases.init([{
    ...
    if (data.moduleOptions) {
        module.config()['core/tokenHandler'] = data.moduleOptions;
    }
...
QUnit.test('getQueueLength', function(assert) {
    const ready = assert.async();
    const tokenHandler = tokenHandlerFactory({ maxSize: 5 });
