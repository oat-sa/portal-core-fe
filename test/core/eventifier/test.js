...
import Promise from 'portal/core/promise';
import testLogger from 'test/core/logger/testLogger';
...
QUnit.test('on context', function(assert) {
    const emitter1 = eventifier();
    const emitter2 = eventifier();
...
QUnit.test('cancel by rejection produces no errors', function(assert) {
    ...
    setTimeout(function() {
        const allTraces = testLogger.getMessages().trace;
        const allErrors = testLogger.getMessages().error;
        const stopTraces = allTraces.filter(function(trace) {
            return trace.stoppedIn;
        });
