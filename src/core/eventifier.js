 ...
 * @returns {eventifier} the target for conveniance
 */
function eventifier(target){
     ...
     */
    const getHandlers = function getHandlers(eventName, type){
    ...
    const eventApi = {
        ...
        trigger : function trigger(eventNames){
            ...
            function triggerHandlers(handlers, event) {
                const pHandlers = handlers.map(function (handler) {
            ...
            function logHandlerStop() {
                // logger has been removed from this simplified eventifier.
            }
            ...
            if(destination && _.isFunction(destination.trigger)){
                ...
                _.forEach(eventNames, function(eventName) {
                    self.on(eventName, function forwardEventTo(){
                        const args = [eventName].concat([].slice.call(arguments));
